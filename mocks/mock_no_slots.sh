#!/usr/bin/env bash
# mock_no_slots.sh
# Mock script to test no available slots template rendering and webhook delivery
...
usage() {
  ...
  echo "  --driver <name>   Driver name to use (default: 'Chris')"
  exit 1
}
...
echo "Testing no_slots template with driver: ${driver}"
echo ""
...
render_status=$?
context="${driver} (no available slots)"
